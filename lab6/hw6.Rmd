...
gabon %>%
  ...
  labs(title="Number of Transects per Land Use Type", x="Land Use Type")
```
...
gabon %>% 
  ...
  labs(title="Vegetation Richness Across Land Use Types", x="Land Use Types", y="Number of Tree Species")
```

The logging type has the highest range and the park type has the highest median. Both logging and neither types seem to have a larger spread of data while the park type is more concentrated.

**7. For which land use type (national parks, logging, or neither) is average all species diversity the greatest?**
```{r}
land_use_and_diversity <- gabon %>% 
  select(land_use, diversity_all_species)
```

```{r}
land_use_and_diversity %>% 
  filter(land_use=="park") %>%
  pull(diversity_all_species) %>% 
  mean()
```
...
The park type had the greatest average all species diversity.
...
gabon %>% 
  ...
  labs(title="Vegetation Richness vs. Mammal Species Richness", x="Number of Tree Species", y="Number of Mammal Species")
```
