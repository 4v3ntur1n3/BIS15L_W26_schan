...
Play with point size by adjusting the `size` argument. #Dot size, too large of a size creates overplotting
```{r}
...
What about just filling in with a color preference or size? #alpha controls transparency
```{r}
p+geom_point(size=3, color="black", alpha=0.8, shape=24, fill="lemonchiffon4")
```

a #keep audience and consistency in mind for choosing color
```{r}
colors()
```
