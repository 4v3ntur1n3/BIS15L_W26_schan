...
```{r}
my_palette <- paletteer_d("vangogh::StarryNight")
my_palette
```
...
```{r}
my_palette <- paletteer_d("futurevisions::jupiter")
q+scale_colour_manual(values=my_palette)
```
...
homerange %>% 
  ggplot(aes(x=taxon, y=log10_mass, fill=thermoregulation))+
  geom_boxplot()+
  facet_wrap(~trophic_guild, ncol=2, labeller=label_both)+ #can do nrow, would look weird
  theme(axis.text.x=element_text(angle=60, hjust=1),
