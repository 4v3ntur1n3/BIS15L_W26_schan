...
ggplot(data = msleep,
       mapping = aes(x = vore))+
  geom_bar(mapping = aes(fill = vore),color="black")+
  labs(title="Number of Mammals in Each Vore Type", x="Vore Type")
```

The herbi vore type is the most represented mammal in this data.
...
ggplot(data = msleep,
  ...
  labs(title="Body Weight of Mammals by Vore Type", x = "Vore Type", y = "Body Weight (log10(kg))")+
  scale_y_log10()
```
...
ggplot(data = msleep,
  ...
  scale_x_log10()+
  labs(title="Mammal Body Weight vs. Total Sleep Time", x="Body Weight (log10(kg))", y="Total Sleep Time (hours)")
```

There is a negative relationship between body weight and total sleep time
...
ggplot(data=msleep,
  ...
  geom_boxplot(mapping=aes(fill=vore))+
  labs(title="Total Hours of Sleep by Vore Type", x="Vore Type", y="Total Sleep Time (hours)")
```
...
ggplot(data=msleep,
  ...
  scale_x_log10()+
  scale_y_log10()+
  labs(title="Mammal Brain Weight vs. Body Weight", x="Brain Weight (log10(kg))", y="Body Weight (log10(kg))")
```

There is a positive relationship with brain weight and body weight of mammals.
...
ggplot(data=msleep,
  ...
  geom_smooth(method=lm, se=T)+
  scale_x_log10()+
  labs(title="Mammal Brain Weight vs. REM Sleep Time", x="Brain Weight (log10(kg))", y="REM Sleep Time (hours)")
```
