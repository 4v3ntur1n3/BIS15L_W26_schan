...
```{r, message = FALSE}
spiders_locations <- read_csv("data/spiders_locations.csv") %>% 
  clean_names()
```

2. Join the files using an appropriate key and join type.
```{r}
spiders_with_locs <- left_join(spider_records, spiders_locations, by="accession")
```

```{r}
names(spiders_with_locs) #check to see if it works
```
