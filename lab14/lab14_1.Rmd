...
latminbear <- 55.02
latmaxbear <- 70.37
lonbear <- c(lonminbear, lonmaxbear)
latbear <- c(latminbear, latmaxbear)
bboxbear <-  make_bbox(lonbear, latbear, f=0.3)
```

3. Load a map from `stadiamaps` in a `stamen_terrain` projection and display the map.
```{r}
mapbear <- get_stadiamap(bboxbear, maptype="stamen_terrain", zoom=5)
```
...
leaflet(grizzly) %>% 
  ...
  addScaleBar(position="bottomleft") %>% 
  fitBounds(lng1=lonminbear, lat1=latminbear,
            lng2=lonmaxbear, lat2=latmaxbear)
```
