...
The study tests if elephant poaching severely decreased the number of elephants with large tusks and if surviving elephant offspring had smaller tusks. The variables involved are historical sample collection year, approximated age, elephant id, shoulder height, elephant sex, tusk length, tusk circumference,
...
elephants %>% 
  ...
  summarize(mean_age_first_range_m=mean(estimated_age_years, na.rm=T),
            median_age_first_range_m=median(estimated_age_years, na.rm=T),
            sd_age_first_range_m=sd(estimated_age_years, na.rm=T))
            #min_age_first_range_m=min(estimated_age_years, na.rm=T),
            #max_age_first_range_m=max(estimated_age_years, na.rm=T))
```

```{r}
elephants %>% 
  filter(years_of_sample_collection=="1966-68" & sex=="f") %>% 
  summarize(mean_age_first_range_f=mean(estimated_age_years, na.rm=T),
            median_age_first_range_f=median(estimated_age_years, na.rm=T),
            sd_age_first_range_f=sd(estimated_age_years, na.rm=T))
```

```{r}
elephants %>% 
  filter(years_of_sample_collection=="2005-13" & sex=="m") %>% 
  summarize(mean_age_second_range_m=mean(estimated_age_years, na.rm=T),
            median_age_second_range_m=median(estimated_age_years, na.rm=T),
            sd_age_second_range_m=sd(estimated_age_years, na.rm=T))
```

```{r}
elephants %>% 
  filter(years_of_sample_collection=="2005-13" & sex=="f") %>% 
  summarize(mean_age_second_range_f=mean(estimated_age_years, na.rm=T),
            median_age_second_range_f=median(estimated_age_years, na.rm=T),
            sd_age_second_range_f=sd(estimated_age_years, na.rm=T))
```

Looking between the years, the age results of females stayed relatively consistent while the mean age of males increased from the first sample period to the next. Looking between sexes, the sampling of males and females from 1966-68 doesn't look even as females had a higher mean and median age than males. The sampling of males and females from 2005-13 looks more even as the mean ages of each sex is approximately the same, though females still have a larger median age.
...
From the plot, age seems like a good positive predictor of tusk length and the trend is consistent with both sexes.
...
From the plot, shoulder height seems like a good positive predictor of tusk length and the trend is consistent with both sexes.
...
elephants %>% 
  ...
  summarize(mean_tusk_length_before=mean(tusk_length_in_cm, na.rm=T))
```

```{r}
elephants %>% 
  filter(years_of_sample_collection=="2005-13") %>% 
  summarize(mean_tusk_length_after=mean(tusk_length_in_cm, na.rm=T))
```

The mean tusk length from 2005-13 is about 20.33% smaller than the mean tusk length from 1966-68.
...
elephants %>% 
  ...
  labs(title="Tusk Length of Mature Elephants Between Sample Periods", x="Sample Periods", y="Tusk Length (cm)")
```

1966-68 had a higher range and median of mature elephants than 2005-13.
