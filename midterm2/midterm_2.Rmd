...
NA's are represented as "unknown" and are distributed in the mating and denning variables.
```{r}
polar_bears <- polar_bears %>% 
  replace_with_na(replace=list(mating="unknown",
                               denning="unknown"))
gg_miss_var(polar_bears)+
  labs(title="Distribution of NA's")
```
...
Counts are even between sexes in the CS population, but there seems to be a little more females than males in the SB population.
```{r}
polar_bears %>% 
  group_by(popn) %>% 
  count(sex, sort=T)
...
polar_bears %>% 
  ggplot(aes(x=sex))+
  geom_bar(aes(fill=popn), position="dodge")+
  labs(title="Polar Bear Counts per Sex Between CS and SB Populations", x="Sex", y=NULL, fill="Population")
```
...
polar_bears_dates %>% 
  ...
  geom_line(aes(color=popn))+
  geom_point()+
  labs(title="Number of Bears Between Populations, from 1983 to 2016", x="Year", y="# of Bears", color="Population")
```
...
```{r}
polar_bears_dates %>% 
  summarize(min_urea=min(urea_nitrogen, na.rm=T),
            median_urea=median(urea_nitrogen, na.rm=T),
            mean_urea=mean(urea_nitrogen, na.rm=T),
...
polar_bears_dates %>% 
  summarize(min_creatinine=min(creatinine, na.rm=T),
            median_creatinine=median(creatinine, na.rm=T),
...
```{r}
polar_bears_dates <- polar_bears_dates %>% 
...
```{r}
polar_bear_counts <- polar_bears_dates %>% 
  select(year, fasting, popn) %>% 
...
polar_bear_counts %>% 
  ...
  geom_line(aes(color=popn))+
  geom_point()+
  labs(title="Number of Fasting Bears Between Populations, from 1983 to 2016", x="Year", y="# of Fasting Bears", filter="Population")
```
...
```
```{r}
latitude <- c(62.46, 72.70)
longitude <- c(-172.2, -134.8)
bbox <- make_bbox(longitude, latitude, f=0.03)
polar_map <- get_stadiamap(bbox, maptype="stamen_terrain", zoom=5)
ggmap(polar_map)+
  geom_point(data=polar_bears_dates, aes(long, lat, color=popn), size=0.75, alpha=0.8)+
  labs(title="Distribution of Polar Bears Between CS and SB Populations", color="Population", x="Longitude", y="Latitude")
```
...
ui <- fluidPage(
  ...
  plotOutput("plot", width="550px", height="450px")
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    polar_bears_dates %>% 
      ...
      geom_boxplot()+
      labs(title="Ratio of Urea to Creatinine Between Populations, Filtering By Different Variables", x="Population", y="Urea to Creatinine Ratio")
  })
}
