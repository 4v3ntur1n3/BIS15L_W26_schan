...
These data aren't perfectly tidy because the variables aren't in lowercase and separated by. Other than how the variable names were formatted
These data are in long format since values of a variable are repeating.

```{r}
sydneybeaches <- sydneybeaches %>% 
  clean_names() #why can both long format and wide format data be tidy?
```
