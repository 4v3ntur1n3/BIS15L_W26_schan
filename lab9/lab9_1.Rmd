...
The income ranges for each religion are separate variables which would make it difficult to plot. Variable names are actually data.
...
relig_income <- relig_income %>% 
  pivot_longer(-religion, #don't move
               ...
               values_to="total") #move count into new column
```
...
Variable names for days is actually data and could be a separate variable.
...
qpcr_tidy %>% 
  ...
  select(gene, experiment=e_number, rep=r_number, mRNA_expression)
